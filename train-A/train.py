...
"""
    ...
    from src.dataset import DataCollatorNoneFilter, AutoregressiveMIDIDataset
    # Load dataset
    from datasets import load_dataset, load_from_disk
    from miditok import REMI
    
    # Initialize REMI tokenizer with single stream mode
    tokenizer = REMI(params="/home/rwkv/cai-RWKV/MIDI-RWKV/train/tokenizer/tokenizer_params.json")
    ds = load_from_disk("/home/rwkv/cai-RWKV/MIDI-RWKV/src/data/prefiltered")

    # 使用自回归数据集 - 单一流模式
    train_data = AutoregressiveMIDIDataset(
        ds["train"],
        tokenizer,
        args.ctx_len,
        ratio_random_tracks_range=(0.4, 1),
        data_augmentation_offsets=(6, 2, 0),
        bos_token_id=tokenizer.vocab.get("BOS_None", None),
        eos_token_id=tokenizer.vocab.get("EOS_None", None)
    )
    
    val_data = AutoregressiveMIDIDataset(
        ds["validation"],
        tokenizer,
        args.ctx_len,
        ratio_random_tracks_range=(0.4, 1),
    ...
    collator = DataCollatorNoneFilter(pad_token_id=tokenizer.pad_token_id, max_length=args.ctx_len)
    data_loader = DataLoader(train_data, shuffle=False, pin_memory=True, batch_size=args.micro_bsz, num_workers=ds_num_workers, persistent_workers=False, drop_last=True, collate_fn=collator)
