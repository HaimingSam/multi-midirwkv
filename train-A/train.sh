...
PROJ_DIR="out/"$CTX_LEN"chi-L"$N_LAYER"-D"$N_EMBD"-"$MODEL_TYPE # set output folder
#
#######################################################################################################################
#
M_BSZ="8" # 减小批次大小以适应更大的模型
LR_INIT="5e-5"
LR_FINAL="1e-6"
#
...
#
python train/train.py --wandb "" --proj_dir $PROJ_DIR \
...
python train/train.py --load_model $PROJ_DIR"/rwkv-4.pth" --wandb "MIDI-RWKV" --proj_dir $PROJ_DIR --my_testing $MODEL_TYPE \
