...
def convert_parquet_to_jsonl(parquet_path, output_dir, tokenizer):
    ...
    output_path = os.path.join(output_dir, "gigamidi-train3.jsonl")
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df)):
            ...
            try:
                with tempfile.NamedTemporaryFile(suffix=".mid", delete=True) as temp_midi_file:
                    ...
                    # Core: Get tokenizer encoding results
                    tokens = tokenizer.encode(temp_midi_file.name)
                    
                    midi_tokens_2d = []
                    if tokenizer.one_token_stream:
                        if tokens.ids: # Check if the token stream is not empty
                            midi_tokens_2d = [tokens.ids]
                    else:
                        # Multi-track mode: Process each track separately
                        for track in tokens:
                            if track.ids: # Only add non-empty tracks
                                midi_tokens_2d.append(track.ids)

                    # Filter empty tracks and write
                    if any(midi_tokens_2d):  # Check if there is at least one non-empty track
                        json_obj = {"midi_tokens": midi_tokens_2d}
...
if __name__ == "__main__":
    ...
    config = TokenizerConfig(
        ...
        one_token_stream_for_programs=False,  # Do not merge all tracks into one token stream
        program_changes=True,  # Allow instrument changes
        one_token_stream=False,  # Multi-track mode
    )
